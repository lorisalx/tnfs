...
void get_file_stats(char* filename, struct stat *buffer); // Get file stats 
void generate_cid_from_file(char* filename, char* cid); // Get CID from a block file
void generate_cid_from_file_content(char* input, size_t input_length, char* cid); // Get CID from a content
void generate_block_name(char* content, int content_size, char* block_name); // Rename a block
